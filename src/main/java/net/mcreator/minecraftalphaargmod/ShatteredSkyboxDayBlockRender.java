...
public class ShatteredSkyboxDayBlockRender implements BlockEntityRenderer<MoonfallSkyBlockDayBlockEntity> {
    ...
    @Override
    public void render(MoonfallSkyBlockDayBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBufferSource, int pPackedLight, int pPackedOverlay) {
        Matrix4f matrix4f = pPoseStack.last().pose();
        renderCube(pBlockEntity, matrix4f, pBufferSource);
    }

    private void renderCube(MoonfallSkyBlockDayBlockEntity entity, Matrix4f matrix, MultiBufferSource bufferSource) {
        // Render each face with its own texture
    VertexConsumer consumer = bufferSource.getBuffer(FakeSkyRegistry.getShatteredDay());
    ...
    private void renderFace(MoonfallSkyBlockDayBlockEntity entity, Matrix4f matrix, VertexConsumer buffer, float f, float g, float h, float i, float j, float k, float l, float m, Direction direction) {
    ...
    private boolean shouldRenderFace(MoonfallSkyBlockDayBlockEntity entity, Direction direction) {
