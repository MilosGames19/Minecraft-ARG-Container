...
public class NoSignalScreen extends Screen {
	...
	private static final boolean ENABLE_BURSTS = true;
	private static final float SPECKLE_CHANCE = 0.01f;
	private static final int GRAIN_SIZE = 4;
	private static final float BURST_PROBABILITY = 0.003f;
	private static final int MAX_PULSE_ALPHA = 0x18;
	private static final float VERTICAL_GLITCH_CHANCE = 0.008f;
	private static final int VERTICAL_GLITCH_WIDTH = 2;
	private static final int VERTICAL_GLITCH_SEGMENT_MIN = 6;
	private static final int VERTICAL_GLITCH_SEGMENT_MAX = 28;
	private static final float HORIZONTAL_GLITCH_CHANCE = 0.01f;
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
		...
		double pulse = Math.abs(Math.sin(timeSeed * 0.015));
		int pulseAlpha = (int) (pulse * MAX_PULSE_ALPHA);
		int pulseGrey = 0x10;
		...
		for (int x = 0; x < this.width; x += GRAIN_SIZE) {
			for (int y = 0; y < this.height; y += GRAIN_SIZE) {
				if (seeded.nextFloat() < SPECKLE_CHANCE) {
					int g = 0x30 + seeded.nextInt(0x30);
					int a = 0x10 + seeded.nextInt(0x20);
		...
		guiGraphics.fill(0, 0, this.width, this.height / 10, (vignetteAlpha << 24) | 0x000000);
		guiGraphics.fill(0, this.height - this.height / 10, this.width, this.height, (vignetteAlpha << 24) | 0x000000);
		guiGraphics.fill(0, 0, this.width / 14, this.height, (vignetteAlpha << 24) | 0x000000);
		guiGraphics.fill(this.width - this.width / 14, 0, this.width, this.height, (vignetteAlpha << 24) | 0x000000);
		...
		guiGraphics.drawCenteredString(this.font, main, centerX + jitterX - 1, centerY - 12 + jitterY, 0xFF8A4A4A);
		guiGraphics.drawCenteredString(this.font, main, centerX + jitterX + 1, centerY - 12 + jitterY, 0xFF4A6A8A);
		guiGraphics.drawCenteredString(this.font, main, centerX + jitterX, centerY - 12 + jitterY, 0xFFDDDDDD);
