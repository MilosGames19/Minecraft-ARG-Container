...
public class DashCooldownOverlay {
	...
	public static void renderDashOverlay(RenderGuiOverlayEvent.Post event) {
		...
		int dashTimer = Math.max(player.getPersistentData().getInt("dashTimer"), (int) Math.round(player.getCapability(TheArgContainerModVariables.PLAYER_VARIABLES_CAPABILITY, null).map(cap -> cap.Cooldown).orElse(0.0)));
		if (dashTimer <= 0) {
			return;
		}
		...
		gui.fillGradient(x - 1, y - 1, x + bw + 1, y + bh + 1, 0xFF202020, 0xFF000000);
		...
		if (fillW > 0) {
			gui.fillGradient(x, y, x + fillW, y + bh, startColor, endColor);
		}
	}
}
