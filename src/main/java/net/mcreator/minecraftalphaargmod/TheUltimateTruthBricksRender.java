...
public class TheUltimateTruthBricksRender implements BlockEntityRenderer<TheUltimateTruthBricksBlockEntity> {
    ...
    private static final ResourceLocation TEXTURE =
            new ResourceLocation("the_arg_container", "textures/ultimatetruth1.png");

    private static VertexBuffer cubeBuffer;

    @Override
    public void render(TheUltimateTruthBricksBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack,
                       MultiBufferSource pBufferSource, int pPackedLight, int pPackedOverlay) {
        if (cubeBuffer == null) {
            org.joml.Matrix4f identity = new org.joml.Matrix4f();
            BufferBuilder bb = Tesselator.getInstance().getBuilder();
            bb.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION);
            PlaneImageRenderer.emitBricksCube(bb, identity);
