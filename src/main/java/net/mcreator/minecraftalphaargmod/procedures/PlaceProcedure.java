...
public class PlaceProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (world instanceof ServerLevel _serverworld) {
			StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("the_arg_container", "asset"));
			if (template != null) {
		...
		TheArgContainerMod.queueServerWork(120, () -> {
		...
		if (entity instanceof Player _player) {
			ItemStack _setstack = new ItemStack(TheArgContainerModItems.VOID_KEY.get()).copy();
