...
public class SpaceDashProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if (McconfigConfiguration.ALPHAVER_LONG_JUMP_ABILITY.get() == true && new Object() {
		...
		}.checkGamemode(entity) && (entity.getCapability(TheArgContainerModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new TheArgContainerModVariables.PlayerVariables())).Cooldown < 1 && entity.isShiftKeyDown()) {
			entity.setDeltaMovement(new Vec3((entity.getDeltaMovement().x() + entity.getLookAngle().x * 1), (entity.getDeltaMovement().y() + entity.getLookAngle().y + 2), (entity.getDeltaMovement().z() + entity.getLookAngle().z * 1)));
