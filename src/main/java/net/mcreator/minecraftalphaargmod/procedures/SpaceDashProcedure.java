...
public class SpaceDashProcedure {
    ...
    public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
        ...
        if (!McconfigConfiguration.ALPHAVER_LONG_JUMP_ABILITY.get()) return;
        if (!isSurvival(entity)) return;
        ...
        if (entity instanceof Player p && p.getFoodData().getFoodLevel() <= 6) return;

        // Capability and cooldown
        TheArgContainerModVariables.PlayerVariables vars = entity
            .getCapability(TheArgContainerModVariables.PLAYER_VARIABLES_CAPABILITY)
            .orElse(null);
        ...
        if (world instanceof Level lvl) {
            ...
            if (sound != null) {
                ...
                TheArgContainerMod.LOGGER.warn("[SpaceDash] Sound not found: the_arg_container:maror");
            }
        }
        ...
        if (serverSide && entity instanceof Player player) {
            ...
            if (!hasInput) moveForward = 1.0; // forward by default
